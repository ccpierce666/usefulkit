...
export function PayRaiseCalculatorTool() {
  ...
  const calc = useMemo(() => {
    ...
    const taxRate = Math.max(0, Math.min(70, effectiveTaxRate)) / 100;
    ...
    const annualNetNow = annualGrossNow * (1 - taxRate);
    const annualNetAfter = annualGrossAfter * (1 - taxRate);
  ...
  }, [currentSalary, currentBonus, effectiveTaxRate, newBonus, raiseAmount, raiseMode, raisePercent]);
