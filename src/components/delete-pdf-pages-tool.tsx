...
export function DeletePdfPagesTool() {
  ...
  const [removedPages, setRemovedPages] = useState(0);

  const rangeHint = useMemo(() => {
    if (!pageCount) return "Upload a PDF to choose which pages to remove.";
    return `PDF has ${pageCount} pages. Example ranges: 1, 3-4, 8-10`;
  }, [pageCount]);
  ...
  async function handleDeletePages() {
    ...
    try {
      ...
      const pagesToDelete = parsePageRanges(pageRange, totalPages);

      if (!pagesToDelete.length) {
        setError("Enter at least one valid page or page range to remove.");
        return;
      }
      ...
      a.href = url;
      a.download = `${file.name.replace(/\.pdf$/i, "") || "document"}-trimmed.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      setRemovedPages(pagesToDelete.length);
    } catch {
      setError("Failed to delete pages. Check the page range and try again.");
    } finally {
  ...
  return (
    <section className="mt-8 grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
      <div className="rounded-3xl border border-line bg-surface p-4 shadow-sm sm:p-6">
        <h2 className="text-xl font-semibold sm:text-2xl">Delete PDF Pages</h2>
        <p className="mt-2 text-sm text-muted">
          Remove selected PDF pages and download a cleaner document without unwanted sheets.
        </p>
        ...
        <label className="mt-4 grid gap-1">
          <span className="text-sm font-semibold text-foreground">Pages To Remove</span>
          <input
            type="text"
            value={pageRange}
            onChange={(event) => setPageRange(event.target.value)}
            placeholder="Example: 1, 3-4, 8-10"
            className="h-11 rounded-xl border border-line bg-white px-3 text-sm outline-none transition focus:border-brand"
          />
          <span className="text-xs text-muted">{rangeHint}</span>
        </label>
        ...
        >
          {isProcessing ? "Removing..." : "Delete Pages and Download"}
        </button>
      ...
      <div className="grid gap-3">
        ...
        <article className="rounded-2xl border border-line bg-surface p-4 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted">Removed Pages</p>
          <p className="mt-2 text-3xl font-bold">{removedPages || "-"}</p>
          <p className="mt-1 text-xs text-muted">Delete one page, several pages, or a whole range at once.</p>
        </article>
      </div>
    </section>
  );
